...
export async function getAccessibleDocument(documentId: string, userId: string) {
  return prisma.document.findFirst({
    ...
    include: { owner: { select: { name: true } }, shares: { select: { userId: true } }, lastSavedBy: { select: { name: true } }, versions: { orderBy: { createdAt: "desc" }, take: 8, select: { id: true, createdAt: true } } },
  });
}
